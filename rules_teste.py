...
def analisar_postura(nariz, quadril_esquerdo, quadril_direito, joelho_esquerdo, joelho_direito):
    ...
    # pontos_sentado = abs(45 - int(anguloJQ))
    # pontos_sentado += -90*abs(centro_quadril.y-centro_joelho.y)
    #print(pontos_deitado, ": P DEITADO\n", pontos_emPe, ": P EM_PE\n", pontos_sentado, ": P SENTADO")
    ...
    if anguloG > 70 and not(abs(centro_quadril.y-centro_joelho.y) <= 0.25):
        if primeiro_quadro:
            tempo_validador = tempo 
            primeiro_quadro = False     
        tempo_validador-=1  
    ...
    if anguloJQ < 45 or abs(centro_quadril.y-centro_joelho.y) <= 0.25:
            if primeiro_quadro:
                tempo_validador = tempo 
                primeiro_quadro = False     
            tempo_validador-=1  
