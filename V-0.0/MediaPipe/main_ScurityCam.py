...
ffmpeg = r"C:\Users\felip\OneDrive\Imagens\kkkk\ffmpeg-8.1-essentials_build\ffmpeg-8.1-essentials_build\bin\ffmpeg.exe"
...
processo = subprocess.Popen([
    ...
    "-i", "rtsp://admin:ulkfyvxl9d@192.168.0.117:554/onvif2",
    "-vf", "fps=15",
    "-f", "rawvideo",
    "-pix_fmt", "bgr24",
    "-"
...
largura, altura = 864, 488
frame_atual = [None]
lock = threading.Lock()

def ler_frames():
    while True:
        raw = processo.stdout.read(largura * altura * 3)
        if len(raw) != largura * altura * 3:
            break
        frame = np.frombuffer(raw, dtype=np.uint8).reshape((altura, largura, 3)).copy()
        with lock:
            frame_atual[0] = frame

t = threading.Thread(target=ler_frames, daemon=True)
t.start()
...
while True:
    ...
    if resultado_maos.multi_hand_landmarks:
        for hand_landmarks in resultado_maos.multi_hand_landmarks:
    ...
    if resultado.pose_landmarks and resultado_maos.multi_hand_landmarks:
        handMonitor = HandMonitor(estado_mao, ombro_esquerdo, ombro_direito, cotovelo_esquerdo, cotovelo_direito, pulso_direito, pulso_esquerdo)
        handMonitor
